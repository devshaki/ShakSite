...
export class ScheduleService {
  ...
  private readonly DAY_MAP: Record<
    DayOfWeek,
    { hebrew: string; number: number }
  > = {
  ...
  private getPeriodFromTemplate(
    templateId: string,
    periodId: string
  ): Period | undefined {
    const template = this.timetable.periodTemplates.find(
  ...
  private buildDisplaySlots(
    ...
    for (const period of template.periods) {
      ...
      if (entry) {
        ...
        if (classInfo) {
          const isBreak =
            period.id.startsWith('B') || entry.classId === 'BREAK';
      ...
      groupA: [
        {
          ...
          isBreak: false,
          label: 'תקשורת נתונים ורשתו (המשך)',
        },
        {
          start: '09:15',
          end: '10:10',
        ...
        },
        {
          ...
          isBreak: false,
          label: 'תקשורת נתונים ורשתו',
        },
        {
          start: '10:55',
          end: '13:35',
          isBreak: false,
          label: "מע' תקשורת נתונים (המשך)",
        },
      ],
      groupB: [
        {
  ...
  private getAllQuotes(): string[] {
    ...
    if (customQuotes) {
      try {
        ...
        const customTexts = parsed.map((q: any) => {
          if (q.author) {
            return `${q.text} — ${q.author}`;
          }
          return q.text;
        });
        return [...this.QUOTES, ...customTexts];
      } catch (e) {
        return this.QUOTES;
      }
    }
    ...
    for (let i = 0; i < timeSlots.length; i++) {
      ...
      if (currentTime >= slot.start && currentTime < slot.end) {
        ...
        if (!nextBreak && i + 1 < timeSlots.length) {
          ...
          const gapMinutes =
            this.timeToMinutes(nextSlot.start) - this.timeToMinutes(slot.end);
          if (gapMinutes > 0) {
            nextBreak = {
              start: slot.end,
              end: nextSlot.start,
